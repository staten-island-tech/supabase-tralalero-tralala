<template>
  <div
    class="bg-gray-900 flex flex-col p-4 mt-4 border border-gray-700 rounded-lg shadow-lg relative"
  >
    <div
      v-if="!auth.id"
      class="absolute inset-0 bg-gray-800 bg-opacity-90 z-10 rounded-lg flex items-center justify-center"
    >
      <p class="text-purple-300 font-medium">Please log in to trade</p>
    </div>

    <h2 class="text-xl font-medium mb-4 text-purple-300">Trade Menu</h2>
    ...
    <div class="">
      <div class="mb-2">
        <input
          ...
          alt="Share Amount"
          :disabled="!auth.id"
          class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 disabled:opacity-70 text-gray-100 placeholder-gray-500"
        />
      </div>
      <div class="mb-2">
        <input
          ...
          alt="Price of Shares"
          :disabled="true"
          class="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-md focus:outline-none focus:ring-1 focus:ring-purple-500 focus:border-purple-500 text-gray-100 placeholder-gray-500"
        />
      </div>
      ...
      <div class="flex space-x-3">
        <button
          ...
          class="w-full bg-green-600 hover:bg-green-500 text-white font-medium py-3 px-4 rounded-md transition duration-200 focus:outline-none focus:ring-1 focus:ring-green-400 focus:ring-offset-1 disabled:opacity-70 disabled:cursor-not-allowed mt-2"
        >
          Buy
        </button>
        ...
        <button
          @click="handleSell"
          :disabled="!isValidAmount || !auth.id"
          class="w-full bg-pink-600 hover:bg-pink-500 text-white font-medium py-3 px-4 rounded-md transition duration-200 focus:outline-none focus:ring-1 focus:ring-pink-400 focus:ring-offset-1 disabled:opacity-70 disabled:cursor-not-allowed mt-2"
        >
          Sell
        </button>
      </div>
      ...
      <div
        v-if="errorMessage"
        class="p-3 bg-red-900 border border-red-700 text-red-100 rounded-md text-sm mt-2"
      >
        {{ errorMessage }}
      </div>
      ...
      <div
        v-if="successMessage"
        class="p-3 bg-green-900 border border-green-700 text-green-100 rounded-md text-sm mt-2"
      >
        {{ successMessage }}
      </div>
    </div>
